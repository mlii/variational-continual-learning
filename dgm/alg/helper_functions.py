...
def sample_bernoulli(mu):
  shape = mu.get_shape()
  return tf.compat.v1.where(tf.random.uniform(shape) < mu, tf.ones(shape), tf.zeros(shape))
...
def log_gaussian_prob(x, mu=0.0, log_sig=0.0):
  ...
  return tf.reduce_sum(input_tensor=logprob, axis=ind)
...
def log_bernoulli_prob(x, p=0.5):
  logprob = x * tf.math.log(tf.clip_by_value(p, 1e-9, 1.0)) \
            + (1 - x) * tf.math.log(tf.clip_by_value(1.0 - p, 1e-9, 1.0))
  ind = list(range(1, len(x.get_shape().as_list())))
  return tf.reduce_sum(input_tensor=logprob, axis=ind)
...
def KL(mu_p, log_sig_p, mu_q, log_sig_q):
  ...
  return tf.reduce_sum(input_tensor=kl, axis=ind)
...
def log_logistic_cdf_prob(x, mu, log_scale):
  ...
  logprob = tf.math.log(1 - tf.exp(-binsize / scale))
...
def log_logit_normal_prob(x, mu, log_sig):
  x_logit = tf.math.log(tf.clip_by_value(x, 1e-5, 1.0 - 1e-5)) \
            - tf.math.log(tf.clip_by_value(1 - x, 1e-5, 1.0 - 1e-5))
  logprob = -(0.5 * np.log(2 * np.pi) + log_sig) \
